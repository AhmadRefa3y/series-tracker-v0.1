...
interface EpisodeClientComponentProps {
  episode: {
...
export default function EpisodeClientComponent({
  episode,
  seriesId,
  imageUrl,
  episodeData,
  isWatched,
}: EpisodeClientComponentProps) {
  const [isPending, startTransition] = useTransition();
  const [optimisticWatched, setOptimisticWatched] = useState(isWatched);

  if (!episodeData) return null;

  const handleMarkWatched = (e: React.FormEvent) => {
    e.preventDefault();
    setOptimisticWatched(true); // Optimistic UI
  ...
  return (
    <form onSubmit={handleMarkWatched} className="w-full sm:w-1/2 lg:w-1/3 p-2">
      <button
        type="submit"
        disabled={optimisticWatched || isPending}
        className={cn(
          "group w-full h-full focus:outline-none transition-transform transform group-hover:scale-105 group-hover:shadow-2xl",
          (optimisticWatched || isPending) && "opacity-60 cursor-not-allowed"
        )}
        tabIndex={0}
      >
        <div
