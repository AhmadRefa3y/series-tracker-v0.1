    ...
    <div className="py-8 px-4 md:px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-8">
        {recentEpisodes.data.map((episode) => (
          <div key={episode.id} className="relative group flex justify-center">
            <Link
              href={`/shows/${episode.Series.title
                .replace(/\s+/g, "_")
                .toLowerCase()}-${episode.Series.seriesTmdbId}`}
              className="relative bg-gray-800 rounded-lg overflow-hidden transform transition-transform duration-200 hover:scale-105 w-full max-w-full"
            >
